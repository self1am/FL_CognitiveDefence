...
class CognitiveAggregationStrategy(fl.server.strategy.FedAvg):
    ...
    Implements OODA loop and MAPE-K framework directly in the aggregation strategy.
    """
    ...
    def aggregate_fit(self, server_round: int, results, failures):
        ...
        try:
            ...
            decisions_dict, explainable_decisions = self.decide(analysis)
            aggregated_params, aggregation_log = self.act(client_updates, decisions_dict)
            ...
            round_metrics = {
                ...
                'num_decisions': len(explainable_decisions),
                'avg_decision_confidence': np.mean([d.confidence for d in explainable_decisions]) if explainable_decisions else 0,
                'defence_strategy': f"Cognitive Defense Strategy (OODA Loop + MAPE-K, threshold={self.anomaly_threshold})"
            }
            
            if self.logger:
                self.logger.log_round_summary(server_round, round_metrics, explainable_decisions)
